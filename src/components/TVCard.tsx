          ...
          <View className="w-[300px] h-[170px] rounded-2xl overflow-hidden bg-dark-800">
            ...
            <View className="absolute bottom-0 left-0 right-0 p-3">
              ...
              <View className="flex-row items-center gap-2 mt-1">
                <Text className="text-dark-300 text-xs">{formatYear(show.first_air_date)}</Text>
      ...
      <Link href={`/tv/${show.id}`} asChild>
        <TouchableOpacity activeOpacity={0.7}>
          <View className="flex-row items-center gap-3 p-2 rounded-xl bg-dark-800/50">
            ...
            <View className={`px-2 py-1 rounded-full ${getRatingBgColor(show.vote_average)}`}>
              <Text className={`text-xs font-bold ${getRatingColor(show.vote_average)}`}>
                {formatRating(show.vote_average)}
              </Text>
            </View>
          </View>
        ...
        <View className="w-[140px] mr-3">
          <View className="w-[140px] h-[210px] rounded-xl overflow-hidden bg-dark-800">
            ...
            <View className="absolute top-2 right-2">
              <View className={`px-1.5 py-0.5 rounded-full ${getRatingBgColor(show.vote_average)}`}>
